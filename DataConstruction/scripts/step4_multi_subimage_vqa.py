#!/usr/bin/env python3
"""Step 4: construct VQA instructions that integrate two or three subimages."""
...
from stages import TEXT_MODEL, load_classification_scores, transform_step4_multi
...
def main() -> None:
    ...
    if not 2 <= args.min_selected <= args.max_selected <= 3:
        parser.error("selection bounds must satisfy 2 <= min-selected <= max-selected <= 3")
    ...
    run_pipeline(args, transform_step4_multi)
