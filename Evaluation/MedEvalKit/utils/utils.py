...
def parse_response(response):
    response = extract_response_after_think(response).lower()
...
def extract_response_after_think(text):
    ...
    if "</think>" not in text:
        return text
    # Some models can emit multiple think blocks; answer is in the tail.
    return text.rsplit("</think>", 1)[-1].strip()


def extract(text, type,hard = True):
    if text:
        text = extract_response_after_think(text)
...
def init_judger():
    judge_type = (os.environ.get("judge_model_type") or "openai").strip() or "openai"
    if judge_type in ["openai", "claude", "deepseek"]:
        judger = openai_llm()
