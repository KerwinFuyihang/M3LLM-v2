...
class InternVL:
    ...
    def process_messages(self,messages):
        ...
        if "image" in messages:
            ...
            max_tiles = int(os.environ.get("internvl_max_tiles_per_image", "6"))
            pixel_value = load_image(image, max_num=max_tiles).to(torch.bfloat16).to("cuda")
            ...
            prompt = prompt + '\n' + text
            pixel_value = [
                load_image(image, max_num=max_tiles).to(torch.bfloat16).to("cuda") for image in images
            ]
