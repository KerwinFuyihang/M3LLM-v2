...
def main():
    ...
    parser.add_argument('--eval_sample_num', type=int, default=0,
                        help='sample size for quick eval (0 means full dataset)')
    parser.add_argument('--eval_sample_seed', type=int, default=42,
                        help='random seed used for eval sampling')
    ...
    # Bash often passes "" for unset vars (e.g. --judge_model_type "$JUDGE_MODEL_TYPE"), which
    # overrides argparse defaults and breaks utils.init_judger().
    def _shell_default(s, default):
        if s is None:
            return default
        s = str(s).strip()
        return s if s else default

    args.use_llm_judge = _shell_default(args.use_llm_judge, "False")
    args.judge_model_type = _shell_default(args.judge_model_type, "openai")
    ...
    os.environ["eval_sample_num"] = str(args.eval_sample_num)
    os.environ["eval_sample_seed"] = str(args.eval_sample_seed)
