def get_model(model_name: str, model_path: str, args):
    ...
    if model_name in ("qwen2.5-vl", "qwen2_5-vl"):
        from model_wrapper.Qwen2_5.qwen2_5_eval import Qwen2_5_VL

        return Qwen2_5_VL(model_path, args)
    if model_name in ("qwen3-vl", "qwen3vl"):
        from model_wrapper.Qwen3_VL.qwen3_vl_eval import Qwen3_VL
    ...
    if model_name in ("medgpt-oss", "medgpt_oss", "medgpt"):
        from model_wrapper.MedGPT_OSS.medgpt_oss_eval import MedGPTOSS

        return MedGPTOSS(model_path, args)
    if model_name in ("hulu-med", "hulumed", "hulu_med"):
        from model_wrapper.HuluMed.hulumed_eval import HuluMed

        return HuluMed(model_path, args)
